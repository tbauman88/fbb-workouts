    ...
    <article className="relative isolate flex flex-col gap-8 lg:flex-row">
      ...
      <div>
        ...
        <div className="group relative max-w-xl">
          <h3 className="mt-3 text-lg/6 font-semibold text-gray-900 group-hover:text-gray-600">{workout.title}</h3>
          {description && (
            <p
              className="mt-5 text-sm/6 text-gray-600"
              dangerouslySetInnerHTML={{
                __html: marked(`${truncatedDescription}${description.length > 150 ? '...' : ''}`)
              }}
            />
          )}
        </div>
